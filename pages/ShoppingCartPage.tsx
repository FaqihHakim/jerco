        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              {cartItems.map(item => {
                ...
                return (
                    <div key={item.id} className={`bg-white rounded-lg border p-4 flex flex-col sm:flex-row items-start gap-4 transition-all ${isSelected ? 'border-gray-200' : 'border-gray-200 bg-gray-50 opacity-70'}`}>
                        ...
                        <div className="flex-1 min-w-0">
                            <div className="flex justify-between items-start">
                                ...
                                <button onClick={() => removeFromCart(item.id)} className="text-red-500 hover:text-red-700 p-1" aria-label="Remove item">
                            ...
                            <div className="flex items-center justify-between mt-4">
                                ...
                                <div className="flex items-center space-x-2">
                                    <button onClick={() => updateCartItemQuantity(item.id, item.quantity - 1)} disabled={item.quantity <= 1} className="p-1.5 rounded-full border border-gray-300 hover:bg-gray-50 disabled:opacity-50">
                                        <Minus className="h-4 w-4" />
                                    </button>
                                    <span className="w-10 text-center font-medium">{item.quantity}</span>
                                    <button onClick={() => updateCartItemQuantity(item.id, item.quantity + 1)} disabled={item.quantity >= item.product.stock_quantity} className="p-1.5 rounded-full border border-gray-300 hover:bg-gray-50 disabled:opacity-50">
