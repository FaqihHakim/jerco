...
const HomePage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      setError(null);
      try {
        const { items: allProducts } = await productService.getAllProducts({ limit: 12 });
        // Mock logic for splitting products
        setNewArrivals(allProducts.slice(0, 4));
        setTopSelling(allProducts.slice(4, 8));

        // Conditionally set recommendations for logged-in customers
        if (user && !isAdmin) {
          const recommendations = allProducts.slice(8, 12).sort(() => 0.5 - Math.random());
          setForYou(recommendations);
        } else {
          // Clear recommendations if user is not a customer or is logged out
          setForYou([]);
        }
      } catch (err: any) {
  ...
  return (
    <div className="space-y-16 md:space-y-24">
      {/* Hero Section */}
      <header className="bg-gradient-to-r from-gray-100 to-gray-200 overflow-hidden">
        <div className="container mx-auto px-4 py-12 md:py-20">
          <div className="grid md:grid-cols-2 gap-8 items-center">
            ...
            <div className="md:w-full flex justify-center items-center h-full">
              <img
                src="https://freepngimg.com/thumb/cristiano_ronaldo/21842-8-cristiano-ronaldo.png"
                alt="Cristiano Ronaldo"
                className="w-full max-w-lg h-auto object-contain drop-shadow-[0_25px_25px_rgba(0,0,0,0.25)]"
              />
            </div>
          </div>
        </div>
